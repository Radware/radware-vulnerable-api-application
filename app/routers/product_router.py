...
async def update_existing_product(
    ...
    for key, value in update_data.items():
        if value is not None: # Ensure we only update fields that were actually passed
...
async def get_product_stock_info(product_id: UUID):
    ...
    if not stock_info:
        # Check if product exists, if not, then product not found, else stock not initialized
