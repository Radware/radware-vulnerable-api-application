...
from .. import db
from ..models.product_models import Product, ProductCreate, ProductInDBBase, ProductUpdate, Stock, StockUpdate, StockInDBBase
from ..models.user_models import User # For potential future owner checks, though not used for BFLA demo
from ..routers.user_router import get_current_user # Re-use existing dependency for consistency
...
async def create_new_product(
    ...
    """
    print(f"Creating new product '{name}'. Intended BFLA: No admin check performed.") # Logging for demo
    new_product_data = ProductCreate(name=name, price=price, description=description, category=category)
    ...
    initial_stock = StockInDBBase(product_id=product_in_db.product_id, quantity=0) # Default to 0 stock
...
async def search_products_by_name(name: str = Query(...)):
    ...
    """
    print(f"Searching for product with name containing: '{name}'. Intended Injection Target.")
    ...
    results = [Product.model_validate(p) for p in db.db["products"] if name.lower() in p.name.lower()]

    # Simulate a scenario where an injection might cause an error or unexpected behavior
    if "'" in name or ";" in name or "--" in name: # Very basic check for typical SQL injection characters
...
async def update_existing_product(
    ...
    category: Optional[str] = Query(None),
    internal_status_param: Optional[str] = Query(None, alias="internal_status"), # Parameter pollution target
    ...
    """
    product_to_update = next((p for p in db.db["products"] if p.product_id == product_id), None)
    if not product_to_update:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found")

    # Use ProductUpdate model to get clean update data
    update_data = ProductUpdate(name=name, description=description, price=price, category=category, internal_status=internal_status_param).model_dump(exclude_unset=True)

    if not update_data:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No update data provided")

    for key, value in update_data.items():
        if value is not None: # Ensure we only update fields that were actually passed (excluding unset)
            setattr(product_to_update, key, value)
            if key == "internal_status":
                print(f"Product {product_id} internal_status being set to '{value}' via query param. Parameter Pollution Target.")
...
async def delete_existing_product(
    ...
    print(f"Deleting product {product_id}. Intended BFLA: No admin check performed.") # Logging for demo
...
async def get_product_stock_info(product_id: UUID):
    ...
    product_exists = next((p for p in db.db["products"] if p.product_id == product_id), None)

    if not product_exists:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found, so no stock information available.")

    if not stock_info:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Stock information not found for this product (possibly not initialized).")
...
async def update_product_stock_quantity(
    ...
    """
    stock_to_update = next((s for s in db.db["stock"] if s.product_id == product_id), None)
    product_exists = next((p for p in db.db["products"] if p.product_id == product_id), None)

    if not product_exists:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found, cannot update stock.")
    ...
    print(f"Updating stock for product {product_id} to quantity {quantity}. Intended BFLA: No admin/owner check performed.") # Logging for demo
